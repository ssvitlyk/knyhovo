...
/** Silence the structured build log in tests. */
const SILENT = { now: NOW, logSink: () => {} };
...
describe('buildHome — composition', () => {
  ...
  it('editorial UNKNOWN provider: an unpriced curated book still composes (storeName null)', async () => {
    const db = withCollections(emptyDb(), 'knyhovyk-radyt');
    db.books.push(book('unpriced', 'Unpriced', 'A', { listings: [] }));
    db.collectionItems.push(...itemsFor('col-knyhovyk', ['unpriced']));
    const home = await buildHome(makeFakePrisma(db), SILENT);
    const knyhovyk = shelfBy(home, 'knyhovyk')!;
    expect(knyhovyk.books.map((b) => b.id)).toEqual(['unpriced']);
    expect(knyhovyk.books[0]?.storeName).toBeNull();
  });
...
describe('GET /api/home — route + cache + wishlist', () => {
  ...
  it('responds { shelves: [{ key, books }] } and is public (guest 200, isWishlisted false)', async () => {
    const app = buildApp(makeFakePrisma(bigDb()));
  ...
  it('guest and auth get the SAME composition; only isWishlisted differs', async () => {
    ...
    const app = buildApp(prisma, makeAuthDeps(prisma, NOW));
  ...
  it('cache hit path: getHome returns identical composition on a warm second call', async () => {
    const prisma = makeFakePrisma(bigDb());
    const first = await getHome(prisma, null);
    const second = await getHome(prisma, null);
