// Knyhovo Wishlist v1.0 — Shared primitives for the implementation-ready package.
      ...
      {mobile ? (
        <div className="v1-wrap">
          ...
          <footer className="v1-footer">
            <img className="v1-footer-logo" src={logo} alt="Knyhovo" />
...
/* ─────────────────── V1MobilePanel — collapsible mobile monitoring ──────── */
function V1MobilePanel({ items }) {
  const [open, setOpen] = React.useState(false);
  const drops = V1_WL.drops(items);
  const digest = drops[0];
  const label = drops.length
    ? drops.length + ' ' + (drops.length === 1 ? 'знижка' : 'знижки') + ' цього тижня'
    : 'Цього тижня без змін';
  return (
    <div className="v1-mob-mon">
      <button className="v1-mob-mon-toggle" type="button"
        onClick={() => setOpen((o) => !o)} aria-expanded={open}>
        <V1_WL.Icon name="chart-line" size={16} />
        <span className="v1-mob-mon-label">Моніторинг</span>
        <span className="v1-mob-mon-hint">{label}</span>
        <V1_WL.Icon name="chevron-down" size={16} />
      </button>
      {open && (
        <div className="v1-mob-mon-body">
          <div className="v1-mob-savings">
            <span className="v1-plabel">ЗАОЩАДЖЕНО</span>
            <span className="v1-savings-amt v1-savings-amt--sm">412 ₴</span>
          </div>
          {digest && (
            <p className="v1-mob-digest">
              «{digest.title}» → <b>{V1_WL.uah(digest.price)}</b> — найнижча за 3 місяці.
            </p>
          )}
          <p className="v1-mob-check">
            <V1_WL.Icon name="clock" size={13} /> Перевірено сьогодні о 08:00
          </p>
        </div>
      )}
    </div>
