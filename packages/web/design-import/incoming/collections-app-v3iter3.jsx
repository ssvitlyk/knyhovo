...
function BookSection({ band, eyebrow, title, sub, fresh, allLabel, allHref, books, badgeFor, saved, onToggle }) {
...
function GenreSection() {
  return (
    <section className="sec reveal">
      <div className="page">
        <SecHead eyebrow="Навігація" title="За жанром" sub="Оберіть напрям — і дивіться найкращі ціни всередині нього." allLabel="Усі жанри" allHref="#/dobirky/zhanry" />
...
function FreshSection() {
  ...
  return (
    <section className="sec reveal">
      <div className="page">
        <SecHead
          eyebrow="Нові добірки цього тижня"
          title="Щойно зібрали"
          sub="Нові тематичні добірки, які Книговик зібрав власноруч цього тижня."
          fresh={{ text: 'Нові добірки регулярно поповнюються', ed: true }}
          allLabel="Архів добірок" allHref="#/dobirky/arkhiv"
        />
        {isMobile ? (
          <div className="list-rows-card">
            {FRESH.map((f) => (
              <ListRow key={f.slug} href={`#/dobirky/${f.slug}`} icon={f.icon} title={f.name} desc={f.desc} count={`${f.count} книг`} />
            ))}
          </div>
        ) : (
...
/* ═══ PAGE — Iteration 3 content rhythm (editorial ▸ shelf ▸ discovery/nav) ═══
   1 Книговик радить (frozen, cream) · 2 Обране читачами (shelf) · 3 Що читати сьогодні (sage) ·
   4 Популярне зараз (shelf) · 5 За жанром (nav) · 6 Новинки місяця (shelf) ·
   7 Щойно зібрали (editorial) · 8 Найбільші знижки (warm shelf) · 9 Недооцінені книги (cool shelf) · Footer */
  ...
  return (
    <div>
      ...
      <main>
        ...
        {/* 2 · Обране читачами — social-proof shelf, directly below the hero (algorithmic) */}
        <BookSection
        ...
        {/* 3 · Що читати сьогодні — mood discovery (editorial, sage band) */}
        <MoodSection />

        {/* 4 · Популярне зараз — trending shelf (algorithmic) */}
        <BookSection
          eyebrow="Найчастіше переглядають"
          title="Популярне зараз"
          sub="Книги, які зараз переглядають найчастіше на Knyhovo."
          fresh={{ text: 'Найчастіше переглядають сьогодні' }}
        ...
        {/* 5 · За жанром — navigation */}
        <GenreSection />

        <SecDivider />

        {/* 6 · Новинки місяця — new-arrivals shelf (algorithmic) */}
        <BookSection
          eyebrow="Свіже на полицях"
          title="Новинки місяця"
          sub="Свіжі надходження за останній тиждень — Knyhovo вже знайшло для них ціни."
        ...
        {/* 7 · Щойно зібрали — weekly editorial collections */}
        <FreshSection />

        {/* 8 · Найбільші знижки — discount shelf (algorithmic, warm band) */}
        <BookSection
          band="warm"
          eyebrow="Вигідно зараз"
          title="Найбільші знижки"
          sub="Найвигідніші пропозиції серед усіх книгарень сьогодні."
        ...
        {/* 9 · Недооцінені книги — hidden-gems editorial band (no shelf, fanned cover trio) */}
        <GemsBand />
      </main>
