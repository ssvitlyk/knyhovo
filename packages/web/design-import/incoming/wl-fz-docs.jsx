...
function V1DocMascotBoard({ theme }) {
  const USES = [
    { key: 'empty',    label: 'Порожній стан',   copy: 'Яку книгу читаєте? Книговик постереже.', ok: true },
    { key: 'first',    label: 'Перша книга',      copy: 'Починаємо з першої книги. Книговик уже стежить.', ok: true },
    ...
    { key: 'unavail',  label: 'Недоступна книга', copy: 'Зараз книги немає в наявності. Книговик повідомить, коли вона повернеться.', ok: true },
  ];
  const NOUSE = [
        ...
        <div className="v1-doc-grid-2" style={{ marginBottom: 40, alignItems: 'start' }}>
          ...
          <figure className="v1-mascot-fig" data-theme="dark">
            ...
            <figcaption>
              ...
              <p>Підлогова лампа, тепле заокруглене світло. Та сама поза, ті ж пропорції. Композиція масштабована на ~11% — однакова візуальна площа hero-зони з еталонною світлою темою.</p>
            </figcaption>
          </figure>
        </div>
      ...
      <div className="v1-doc-wrap">
        ...
        <DocSection title="Ієрархія інформації" theme={theme}>
          <ol className="v1-doc-ol">
            <li><b>Книги користувача</b> — основний і єдиний контент-стовпець (single-column, max-width ~940px).</li>
            <li><b>Сигнали довіри</b> — savings pill + check-signal у шапці сторінки (не окрема панель).</li>
            <li><b>Цінові можливості</b> — drop-banner та акцентовані рядки з CTA.</li>
            <li><b>Фільтри та сортування</b> — рядок над списком книг.</li>
          </ol>
        </DocSection>
        ...
        <DocSection title="Сітка та відступи" theme={theme}>
          <div className="v1-doc-grid-2">
            <div>
              ...
              <Spec label="Контент">Один стовпець, max-width ~940px, без правої панелі</Spec>
        ...
        <DocSection title="Адаптивна поведінка" theme={theme}>
          <div className="v1-doc-grid-2">
            {[
              ['≥1024px', 'Desktop: один контент-стовпець (max-width ~940px). Без правої панелі.'],
              ['768–1023px', 'Tablet: той самий один стовпець, рядки книг на повну ширину.'],
              ['<768px', 'Mobile: одна колонка, компактний summary-рядок, sticky CTA, 44px touch targets.'],
            ].map(([bp, desc]) => (
              <div key={bp}>
    ...
    <div className="v1-doc-page v1-doc-page--approval" data-theme={theme} data-screen-label="Doc · Approval">
      <div className="v1-doc-wrap v1-doc-wrap--narrow">
        <p className="v1-eyebrow" style={{ letterSpacing: '0.12em' }}>
          KNYHOVO · ВІШЛИСТ v1.0 · FINAL DESIGN FREEZE
        </p>
        <h1 className="v1-h1 v1-h1--large">
          Вішлист v1.0 — фінальна фіксація
        </h1>
        ...
        <div className="v1-approval-why">
          ...
          <div className="v1-doc-grid-2">
            {[
              ['Варіант C як основа', 'Чистий односмуговий (single-column) layout: список книг — єдиний фокус. Праву панель моніторингу прибрано. Низьке когнітивне навантаження.'],
              ['D-елементи для цінності', 'Лічильник заощаджень і сигнал «перевірено сьогодні» у шапці роблять цінність Knyhovo видимою, не нав\'язливою. Без окремих дашборд-карток.'],
        ...
        <div className="v1-approval-frozen">
          ...
          <ol className="v1-doc-ol">
            <li>Односмугова (single-column) сітка на desktop — незмінна структура. Праву панель моніторингу прибрано в усіх станах.</li>
            <li>Жодних дашборд-карток: ні панелі моніторингу, ні картки заощаджень, ні картки дайджесту.</li>
            <li>Savings counter і last-check signal — компактні сигнали в шапці (не окрема панель).</li>
        ...
        <div className="v1-approval-stamp">
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 8 }}>
            Approved for implementation · Knyhovo Design System v1.0 · Без подальших досліджень і альтернатив
          </p>
        </div>
      </div>
    </div>
...
/* ── Offer card specification (freeze) ────────────────────── */
function V1DocCardSpec({ theme }) {
  const items = _DW.getItems('Хвиля знижок');
  const drop = items.find((i) => _DW.delta(i) < 0) || items[0];
  const plain = items.find((i) => _DW.delta(i) === 0 && i.avail === 'in') || items[1];
  return (
    <div className="v1-doc-page" data-theme={theme} data-screen-label="Doc · Card spec">
      <div className="v1-doc-wrap">
        <p className="v1-eyebrow">СПЕЦИФІКАЦІЯ КОМПОНЕНТА · FINAL DESIGN FREEZE</p>
        <h1 className="v1-h1">Картка пропозиції</h1>
        <p className="v1-doc-lead">
          Desktop-рядок і компактна мобільна картка. Мобільна картка скорочена на ~28%:
          вторинна інформація перенесена у розгортуваний блок.
        </p>

        <h3 className="v1-doc-h3">Desktop · рядок</h3>
        <div style={{ maxWidth: 880, marginBottom: 16 }}>
          <_DV.Row item={drop} />
        </div>
        <div className="v1-spec-list" style={{ marginBottom: 40 }}>
          <Spec label="Сітка">48px · 1fr · auto · auto · auto · gap var(--space-4)</Spec>
          <Spec label="Badge">Максимум один: Вигідний момент (green) → -N% (solid) → Не в наявності (neutral)</Spec>
          <Spec label="Умова «Вигідний момент»">Ціна на історичному мінімумі або суттєво нижче середньої. Тон оптимістичний — не FOMO</Spec>
          <Spec label="CTA">primary sm лише для вигідного моменту; secondary sm — решта доступних</Spec>
          <Spec label="Ціна">Serif --accent 1.5rem · стара ціна muted strikethrough · книгарня --text-muted (frozen ієрархія)</Spec>
        </div>

        <h3 className="v1-doc-h3">Mobile · компактна картка (згорнута за замовчуванням)</h3>
        <div className="v1-doc-grid-2" style={{ alignItems: 'start' }}>
          <div style={{ maxWidth: 390, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <_DV.MobCard item={drop} />
            <_DV.MobCard item={plain} />
          </div>
          <div className="v1-spec-list">
            <Spec label="Згорнуто">Обкладинка 36×52 · назва · автор · статус («Стежимо» / «Перевірено сьогодні») · ціна · дельта</Spec>
            <Spec label="CTA">Видимий без розгортання лише для вигідного моменту</Spec>
            <Spec label="Розгорнуто">Стара ціна · книгарня · сповіщення · цільова ціна · дії</Spec>
            <Spec label="Висота">~96px згорнуто (−25–30% від чернетки v1.0) · тап-ціль ≥44px</Spec>
            <Spec label="Взаємодія">Тап по картці розгортає деталі; chevron обертається 180°</Spec>
          </div>
        </div>
      </div>
    </div>
...
Object.assign(window, { V1DocPanelSpec, V1DocMascotBoard, V1DocAnnotations, V1DocApproval, V1DocCardSpec });
