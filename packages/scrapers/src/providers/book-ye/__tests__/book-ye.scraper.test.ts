...
import { BookYeScraper } from '../book-ye.scraper.js';
...
describe('BookYeScraper.scrape — successful pages', () => {
  it('returns combined listings from multiple pages', async () => {
    ...
    expect(result.provider).toBe('book-ye');
  ...
  it('deduplicates listings with identical URLs across pages', async () => {
    const catalogPage = loadFixture('catalog-page.html');
  ...
  it('requests Magento-style ?p=N pagination URLs', async () => {
    const fetcher = makeFetcher([loadFixture('catalog-page.html'), loadFixture('catalog-empty.html')]);
    const scraper = new BookYeScraper(fetcher, 'https://book-ye.com.ua/catalog/');
    await scraper.scrape({ delayMs: 0 });

    expect(fetcher.fetch).toHaveBeenNthCalledWith(1, 'https://book-ye.com.ua/catalog/?p=1', expect.any(Number));
    expect(fetcher.fetch).toHaveBeenNthCalledWith(2, 'https://book-ye.com.ua/catalog/?p=2', expect.any(Number));
  });
...
describe('BookYeScraper.scrape — network errors', () => {
...
describe('BookYeScraper.scrape — ScraperResult shape', () => {
